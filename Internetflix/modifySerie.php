...
<head>
    ...
    <title>Internetflix sorozat módosítása</title>
    <link rel="stylesheet" href="index.css">
</head>
<body>
    <h1>INTERNETFLIX - sorozat módosítása</h1>
    ...
    <form action="" method="post" class="form" novalidate>
        Sorozat címe: <input type="text" name="title" id="" value="<?=$_POST['title'] ?? $serie['title'] ?? $_POST['title'] ?? ''?>"><span><?=$errors['title'] ?? ''?></span><br>
        Megjelenés éve: <input type="number" name="year" id="" value="<?=$_POST['year'] ?? $serie['year'] ?? ''?>"><span><?=$errors['year'] ?? ''?></span><br>
        Cselekmény: <input type="text" name="plot" id="" value="<?=$_POST['plot'] ?? $serie['plot'] ?? ''?>"><span><?=$errors['plot'] ?? ''?></span><br>
        Poszter: <input type="url" name="cover" id="" value="<?=$_POST['cover'] ?? $serie['cover'] ?? ''?>"><span><?=$errors['cover'] ?? ''?></span><br>
